...
void QCPPDialogImpl::saveSettings()
{
    ...
    settings.setValue("/cutecom/Databits", m_dataBitsCb->currentText());
    settings.setValue("/cutecom/Parity", m_parityCb->currentText());
    settings.setValue("/cutecom/Stopbits", m_stopCb->currentText());
...
void QCPPDialogImpl::sendFile()
{
             ...
             || (m_protoPb->currentText()=="1kXModem"))
    {
        ...
        connect(m_sz, SIGNAL(stateChanged(QProcess::ProcessState)), this, SLOT(sendDone(QProcess::ProcessState)));
        //FIXME: how to know if the process runned successfully
        m_sz->start("sh", QIODevice::ReadOnly);
        /*if (!)
        {
...
void QCPPDialogImpl::killSz()
{
    ...
    //FIXME: how to terminate a process
    //m_sz->tryTerminate();
}
...
void QCPPDialogImpl::sendDone(QProcess::ProcessState state)
{
...
bool QCPPDialogImpl::eventFilter(QObject* watched, QEvent *e)
{
    ...
    {
        //FIXME: ke->event() modified for ke->modifiers()
